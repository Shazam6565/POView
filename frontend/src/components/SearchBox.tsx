...
export default function SearchBox({ onSearch, onRecenter, onClear, onToggleLayers, isAnalyzing, layersVisible }: SearchBoxProps) {
    ...
    return (
        <div ref={wrapperRef} className="relative z-50 w-full">
            <form onSubmit={handleSubmit} className="flex flex-col space-y-4 bg-black/40 backdrop-blur-2xl border border-white/10 rounded-2xl p-4 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] transition-all focus-within:border-white/20">
                ...
                <div className="flex items-center bg-white/5 border border-white/10 rounded-xl focus-within:bg-white/10 focus-within:border-white/30 transition-all p-2">
                    <Search className={`w-5 h-5 ml-2 ${isAnalyzing ? 'text-cyan-400 animate-pulse' : 'text-white/50'}`} />
                    <input
                        type="text"
                        value={intentValue}
                ...
                <div className="flex items-center space-x-4 px-2">
                    <span className="text-xs text-white/60 font-bold uppercase tracking-wider w-16">Radius</span>
                    <input
                        type="range"
                        min="0.1"
                        max="1.0"
                    ...
                    />
                    <span className="text-xs text-white uppercase font-bold w-12 text-right">{radiusValue.toFixed(1)} mi</span>
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 pt-2">
                    <button
                        type="submit"
                        disabled={isAnalyzing || !intentValue.trim() || (!selectedPlaceId && !inputValue.trim())}
