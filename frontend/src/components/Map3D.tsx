...
export default function Map3D({
  ...
  const cameraConfig = useMemo(() => {
    ...
    if (recenterTrigger && recenterTrigger > 0) {
      ...
      const targetLat = location?.lat || 40.73;
      const targetLng = location?.lng || -73.995;
    ...
    if (
      ...
      selectedRecommendation.lng
    ) {
      // Priority 0: Explicitly clicked recommendation
      return {
        destination: Cartesian3.fromDegrees(
          selectedRecommendation.lng,
          selectedRecommendation.lat - 0.001,
          40,
        ), // Cinematic "Hero View"
        orientation: {
          heading: 0,
    ...
    return {
      destination: Cartesian3.fromDegrees(-73.995, 40.73, 1200),
      orientation: {
        heading: CesiumMath.toRadians(30),
  ...
  return (
    <div className="w-full h-full relative">
        ...
        ref={(e: { cesiumElement?: unknown } | null) => {
          if (e && e.cesiumElement) {
            ...
            if (!viewerInitialized.current) {
              ...
              initSpatialPerception(
                e.cesiumElement as CesiumViewer,
                (telemetry, visiblePOIs) => {
      ...
      >
        ...
        <Cesium3DTileset url={GOOGLE_TILE_URL} showCreditsOnScreen={true} />
        ...
        {layersVisible &&
          recommendations.map((rec, index) => {
            ...
            const pathPositions = rec.routingPath
              ? getPolylinePositions(rec.routingPath)
              : null;
            return (
              <React.Fragment key={`rec-${index}`}>
                ...
                {/* Vertical identifier line — ground to pin */}
                <Entity>
                  <PolylineGraphics
        ...
        {layersVisible &&
          ...
          recommendations.length === 0 && (
            <Entity
              position={Cartesian3.fromDegrees(location.lng, location.lat, 20)}
            >
              <PointGraphics
                pixelSize={12}
                color={Color.WHITE}
      ...
      <div className="absolute bottom-10 right-10 z-50 flex flex-col space-y-2 bg-black/40 backdrop-blur-md rounded-xl p-2 border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)]">
        ...
        >
          <svg
            ...
            className="opacity-70 group-hover:opacity-100 group-hover:text-cyan-400"
          >
