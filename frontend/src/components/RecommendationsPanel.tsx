                ...
                {profileData && (
                    <div className="mb-6 pb-6 border-b border-white/5">
                        <div className="flex items-center justify-between mb-2">
                            <h3 className="text-white/60 text-xs font-semibold uppercase tracking-[0.2em]">Contextual Insight</h3>
                            {profileData.weather && (
                                <span className="bg-black/30 border border-white/10 px-3 py-1.5 rounded-full text-xs font-mono font-medium text-white/90 flex items-center space-x-2 whitespace-nowrap overflow-hidden text-ellipsis max-w-[180px] shadow-sm" title={profileData.weather.ai_summary}>
                        ...
                        <h4 className="text-white text-xl font-bold tracking-tight mb-3 drop-shadow-sm">{profileData.neighborhood_name}</h4>
                        <p className="text-white font-medium text-base/relaxed drop-shadow-sm">{profileData.vibe_description}</p>
                    </div>
                )}
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-white tracking-tight">AI Targeting</h2>
                        ...
                        <div className="pl-4">
                            ...
                            <div className="flex items-center space-x-3 mt-4 pt-4 border-t border-white/10">
                                <div className="flex items-center space-x-1.5 bg-black/30 pl-1.5 pr-2.5 py-1 rounded-full border border-white/10 shadow-sm">
                                    ...
                                    <span className="text-white font-mono text-xs font-semibold">{rec.rating.toFixed(1)}</span>
                                </div>
